...
│   └── frontend/
│       └── package.json
├── demo/gradio_app.py          # 当前可直接运行的前端 Demo
├── configs/
│   ├── sft_config.yaml        # 历史 SFT 配置草稿
│   ├── grpo_config.yaml       # 历史 TRL 配置草稿
...
说明：`training/sft_coldstart.py`、`training/grpo_train.py`、
`training/grpo_train_unsloth.py`、`training/void_turn_filter.py`、
`docker-compose.yml` 当前不在仓库中，不能再视为“现成可运行文件”。
...
> 2. `scripts/generate_sft_data.py`、`training/sft_coldstart.py`、
>    `scripts/test_sft_model.py` 当前仓库中不存在。
> 3. `configs/sft_config.yaml` 仍保留为配置占位，但不能单独构成一条可执行 SFT pipeline。
...
SFT 冷启动在设计上仍然合理，但需要明确它在当前仓库里并未落地为可执行脚本：

- 已存在：`configs/sft_config.yaml`
- 不存在：`scripts/generate_sft_data.py`
- 不存在：`training/sft_coldstart.py`
- 不存在：`scripts/test_sft_model.py`

结论：本章保留 SFT 作为未来扩展方向，但不要再把它视为当前仓库的默认训练步骤。
...
- `scripts/train_grpo_verl.sh`
- `configs/verl_ppo_finqa.yaml`

核心调用链如下：

1. `training/search_r1_compat.py` 把 `vendor/Search-R1` 加入 `sys.path`
2. `training/finagent_verl_main.py` 将 FinAgent 的 generation manager 注入 veRL trainer
3. `training/finagent_generation.py` 负责多轮 `<search>/<calculate>/<sql>/<answer>` 交互
4. `training/reward_functions.py` 负责 rule-based reward
...
当前多轮 Agent 训练的关键不在 `trl.GRPOTrainer`，而在自定义 generation manager：

- `training/finagent_generation.py` 解析模型输出动作
- 支持四类动作：`search`、`calculate`、`sql`、`answer`
- 每轮工具执行后，把结果包装为 `<observation>...</observation>` 再喂回模型

这才是 FinAgent 与单轮 RL 示例最大的差别。

已知注意事项：

- `tools/tool_dispatcher.py` 在评测和 Demo 路线中会把 `question_id` 传给 SQL 工具
- 但 `training/finagent_generation.py` 当前仍直接调用 `execute_sql(content)`
- 这意味着“question_id -> table_name 自动路由”在 veRL 训练路径里只算部分实现
...
因此旧版“Unsloth 主线”与“纯 TRL 备选”应视为设计草稿，而不是当前仓库可直接执行的训练入口。
...
当前真正可运行的前端是 `demo/gradio_app.py`。

`demo/frontend/` 目录下目前只有 `package.json` 脚手架，
并没有完整的 React `src/` 实现，因此不能再把 React 前端视为“已完成模块”。
...
因此 Docker Compose 仍是待补全项，不属于当前已实现功能。
