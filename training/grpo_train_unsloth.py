"""Compatibility GRPO training entrypoint built on Unsloth + TRL."""
...
def main() -> None:
    ...
    try:
        ...
        raise RuntimeError("Unsloth is not installed. Install it or use training/grpo_train.py instead.") from exc

    model, tokenizer = FastLanguageModel.from_pretrained(
        model_name=model_path,
        max_seq_length=int(config["training"]["max_prompt_length"]) + int(config["training"]["max_completion_length"]),
        load_in_4bit=True,
        dtype=None,
    )
    ...
    final_dir = Path(config["output_dir"]) / "final_unsloth"
    final_dir.mkdir(parents=True, exist_ok=True)
    trainer.save_model(str(final_dir))
    tokenizer.save_pretrained(str(final_dir))
    print(f"Saved Unsloth GRPO checkpoint to {final_dir}")
